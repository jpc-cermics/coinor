...
int int_clp_solve(Stack stack, int rhs, int opt, int lhs)
{
  ...
  if ( GetArgs(stack,rhs,opt,T,&Objective, &ObjA, &Rhs, &ObjAe, &Rhse, &opts,
	       &ObjQ,&lb,&ub,&sense_str,&var_type,&Options,&save_only,&filename) == FAIL) 
    return RET_BUG;
  
  if ( filename != NULL ) 
    {
      const char *extension = nsp_get_extension(filename);
      if ( extension != NULL && !strcmp(extension,".mps") ) 
	{
	  Scierror("Error: file extension should be .mps");
	  return RET_BUG;
	}
...
int int_coinmp_solve(Stack stack, int rhs, int opt, int lhs)
{
  ...
  if ( GetArgs(stack,rhs,opt,T,&Objective, &ObjA, &Rhs, &ObjAe, &Rhse, &opts,
	       &lb,&ub,&sense_str,&var_type,&Options,&SemiCont,&save_only,&filename) == FAIL) 
    return RET_BUG;
...
extern function int_coinmp_readlp;

static OpTab libcoinor_func[] = {
  {"clp_sparse", int_clp_sparse},
  {"clp_sparse2", int_clp_sparse2},
  {"linprog_clp", int_clp_solve},
  {"linprog_coinmp", int_coinmp_solve}, /* this is to use cbc */
  {"coinmp_options", int_coinmp_options},
  {"readlp_coinor", int_coinmp_readlp},
  {(char *) 0, NULL},
};

int libcoinor_Interf (int i, Stack stack, int rhs, int opt, int lhs)
{
  return (*(libcoinor_func[i].fonc)) (stack, rhs, opt, lhs);
}

void libcoinor_Interf_Info (int i, char **fname, function (**f))
{
 *fname = libcoinor_func[i].name;
 *f = libcoinor_func[i].fonc;
}
